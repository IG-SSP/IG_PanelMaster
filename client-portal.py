...
.network-strip{{position:relative;height:76px;margin-bottom:22px;overflow:hidden;border:2px solid var(--line);background:linear-gradient(180deg,#030c19,#071c31);box-shadow:7px 7px 0 var(--shadow)}}.network-strip:before{{content:"";position:absolute;inset:0;background-image:radial-gradient(circle,var(--sun) 1px,transparent 2px);background-size:37px 31px;opacity:.55}}.network-line{{position:absolute;left:6%;right:6%;top:37px;height:4px;background:repeating-linear-gradient(90deg,#245f90 0 10px,transparent 10px 17px)}}.pixel-node{{position:absolute;top:27px;width:24px;height:24px;border:3px solid var(--sun);background:#051326;box-shadow:4px 4px 0 var(--shadow)}}.pixel-node.n1{{left:8%}}.pixel-node.n2{{left:38%}}.pixel-node.n3{{right:8%}}.data-packet{{position:absolute;top:32px;left:9%;width:12px;height:12px;background:var(--pink);box-shadow:0 0 8px var(--pink);animation:packet 5s steps(24,end) infinite}}
...
@keyframes spin{{to{{transform:rotate(360deg)}}}}@keyframes rise{{from{{opacity:0;transform:translateY(8px)}}to{{opacity:1;transform:none}}}}@keyframes sheen{{0%,55%{{transform:translateX(-100%)}}85%,100%{{transform:translateX(100%)}}}}@keyframes pulseBorder{{0%,100%{{border-color:#2b78aa}}50%{{border-color:var(--sun)}}}}@keyframes packet{{0%{{left:9%}}45%{{left:49%}}55%{{left:49%}}100%{{left:89%}}}}@keyframes blink{{50%{{opacity:.35}}}}@keyframes load{{from{{width:0}}}}.card,form,.metric,.protocol-card,.guide-card{{animation:rise .28s steps(5,end) both}}
@media(max-width:860px){{main{{padding:12px 13px 40px}}.site-head{{align-items:flex-start}}.site-status{{display:none}}h1{{font-size:34px}}.hero,.grid,.profile,.steps,.guide-grid,.protocol-choice,.protocol-grid,.admin-top,.admin-grid,.support{{grid-template-columns:1fr}}.request-card{{grid-template-columns:1fr}}.fund-panel{{border-left:0;border-top:2px dashed #245f90;padding:16px 0 0}}}}@media(max-width:480px){{.network-strip{{height:60px}}.network-line{{top:29px}}.pixel-node{{top:19px}}.data-packet{{top:24px}}.fund-meta{{display:block}}.fund-meta span{{display:block;margin-top:4px}}button,.btn{{width:100%;text-align:center}}.actions,.mini-actions,.fund-actions{{display:grid;grid-template-columns:1fr}}}}@media(prefers-reduced-motion:reduce){{*,*:before,*:after{{animation:none!important;transition:none!important}}}}
</style></head><body><main>
<header class="site-head"><a class="brand" href="{public_url('/')}"><span class="brand-mark">IG</span><span>SPACE // VPN</span></a><span class="site-status"><i class="status-led"></i> сеть активна</span></header>
<div class="network-strip" aria-hidden="true"><div class="network-line"></div><i class="pixel-node n1"></i><i class="pixel-node n2"></i><i class="pixel-node n3"></i><i class="data-packet"></i></div>
{body}</main><script>
